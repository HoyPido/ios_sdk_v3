...
if [ ! -z "$excludedPathsFromCoverage" -a "$excludedPathsFromCoverage" != " " ]; then
	rm -f tmpFileRunSonarSh2
	FILES=($excludedPathsFromCoverage)
	for f in "${FILES[@]}"
	do
  		echo "\"${f}\"" | tr '\n' ' '>> tmpFileRunSonarSh2
	done
...
if [ "$lizard" = "on" ]; then
	if hash $LIZARD_CMD 2>/dev/null; then
  		$LIZARD_CMD --verbose --xml --exclude "$excludedPathsFromCoverage" "$srcDirs" > sonar-reports/lizard-report.xml
  	else
